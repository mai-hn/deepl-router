...
def test_quota_endpoint_persists_quota(tmp_path, monkeypatch):
    temp_store = Store(tmp_path / "router.db")
    ...
    async def fake_quota(item):
        quota = {"type": "characters", "used": 120, "limit": 500000}
        temp_store.set_quota(item["id"], quota)
        return {"provider_id": item["id"], "quota": quota}

    monkeypatch.setattr(application, "query_provider_quota", fake_quota)
    response = TestClient(application.app).post(f"/api/providers/{provider['id']}/quota")
    assert response.status_code == 200
    saved = temp_store.provider(provider["id"])
    assert saved["quota"] == {"type": "characters", "used": 120, "limit": 500000}
